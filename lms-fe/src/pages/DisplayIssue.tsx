...
const DisplayIssue: React.FC = () => {
  ...
  const filteredBooks = issuedBooks.filter(issue =>
    ...
    (issue.student && issue.student.name.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  return (
    <div className="mt-20 flex flex-col">
      <h2 className="text-2xl font-bold mb-4 text-center underline">Issued Books</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginatedBooks.length === 0 ? (
          ...
          paginatedBooks.map((issue) => (
            ...
            >
              <h3 className="text-lg font-semibold mb-2">{issue.book.title}</h3>
